...
/**
 * Continuous TS-Path semantics (CONT) implemented via STOMP.
 *
 * Mirrors TimeWalkPathCont exactly, replacing TD_JOINOptimized with
 * StompWithAllen:
 *
 *   - Hop 1  : StompWithAllen.myStomp evaluated freely over (ts1, ts2).
 *              Result is cached (key = source->target|tsType|r|w|threshold).
 *
 *   - Hop i≥2: The source subsequence is PINNED to the previous hop's
 *              target subsequence I_{i-1}^{(2)}. A pinned-source variant
 *              of myStomp then searches only over target candidates in ts2
 *              that stand in relation R with the pinned source.
 *              NOT cached — the pinned source varies per path.
 *
 * If any hop returns null the entire path is invalid (strict, no backtracking).
 */
public class StompTimeWalkPathCont {
    ...
    @Procedure(name = "edge2Time.StompTimeWalkPathCont", mode = Mode.READ)
    public Stream<Result> stompTimeWalkPathCont(
            @Name("Path")              Path   p,
    ...
    public void getContPaths(Path p, String tsType, String r, long window,
        ...
        // Stores the absolute start index in τ_{prev} of the subsequence
        // that must be used as the source of the current hop.
    ...
    HopResult getSingleHopPinned(Node source, Node target,
        ...
        if (pinnedSrcIndex == null) {
            ...
            if (cached != null) {
                ...
                ap = st.myStomp(ts1, ts2, (int) window, r);
            ...
            // The source is the single subsequence ts1[pinnedSrcIndex : +window]
            List<DataPoint> pinnedSource =
                    ts1.subList(pinnedSrcIndex, pinnedSrcIndex + (int) window);

            StompWithAllen st = new StompWithAllen();
            ap = st.myStompPinned(pinnedSource, ts2, (int) window, r);
    ...
    // ------------------------------------------------------------------
    //  Cache key — same format as TimeWalkPathCont
    // ------------------------------------------------------------------
