...
public class StompWithAllen {

    public Map<String, List<double[]>> myStomp(
            List<DataPoint> T_A,
            List<DataPoint> T_B,
            int m,
...
//        }
        ...
        // No (i,j) search, no STOMP call per pair — compute the unique
        // candidate j for each i directly from the time grid, and only
        // run STOMP on that single candidate (if any).
        //   - free (this method): O(L_A - w + 1) STOMP calls at most.
        //   - pinned (myStompPinned): O(1) STOMP calls at most.
        // ---------------------------------------------------------------
        ...
        for (int i = 0; i < subseq_T_A.size(); i++) {
            ...
            for (int j = 0; j < subseq_T_B.size(); j++) {
                ...
                var mp = STOMP.of(seqAValues, seqBValues, m);
    ...
    // ------------------------------------------------------------------
    //  Pinned-source STOMP join (CONT recursive hop, hop i >= 2)
    //
    ...
    //  The caller (StompTimeWalkPathCont.getSingleHopPinned) remaps it
    //  to the absolute position in the original series.
    // ------------------------------------------------------------------

    public Map<String, List<double[]>> myStompPinned(
        ...
        // ---- Compatibility check (commented, see myStomp) ----------------
...
//        }
        ...
        for (int j = 0; j < subseq_T_B.size(); j++) {
            ...
            // compute distance using STOMP — identical to myStomp
            double[] seqBValues = seqB.stream().mapToDouble(DataPoint::getValue).toArray();
            var mp = STOMP.of(seqAValues, seqBValues, m);
            double dist = Arrays.stream(mp.profile()).min().orElse(Double.POSITIVE_INFINITY);
    ...
    //  STOMP matrix-profile call for that single candidate, instead of
    //  for every (i,j) pair.
    //
    //  Complexity:
    //    - building startIndexB: O(L_B - w + 1)
    //    - pinned (nA = 1):       O(1) STOMP calls
    //    - free   (nA = L_A-w+1): O(L_A - w + 1) STOMP calls
    // ------------------------------------------------------------------
    ...
    private void computeDirectIndexJoin(
        ...
        for (int i = 0; i < subseq_T_A.size(); i++) {
            ...
            // Only now do we pay for the STOMP matrix-profile computation —
    ...
    // Updated to use primitive arrays
